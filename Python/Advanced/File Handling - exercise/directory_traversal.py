...
for extension, files in extensions:  # extension => py, files => [one.py, two.py]...
    result.append(f".{extension}") # add the extension to the result

    for file in sorted(files): # iterations through all files
        result.append(f"- - - {file}") # add the file's name to the result
